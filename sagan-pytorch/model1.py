...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=[3, 3],
                 ...
                 activation=F.relu, upsample=True, convolution=True, self_attention=False, deform=False):
        super().__init__()
        ...
        self.deform = deform
        if self.deform:
            self.deform = DeformConv2d(in_channel, out_channel, 3, padding=1, bias=False, modulation=False)
...
class Discriminator(nn.Module):
    def __init__(self, n_class=10):
        ...
        def conv(in_channel, out_channel, stride=2,
                 self_attention=False, deform=False):
            return ConvBlock(in_channel, out_channel, stride=stride,
                             bn=False, activation=leaky_relu,
                             upsample=False, self_attention=self_attention, deform=deform)
